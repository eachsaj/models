# ==============================================================================
# Replace the original queue_runner_impl.py file in tensorflow/python/training/
# with this file.
# ==============================================================================
...
class QueueRunner(object):
  ...
  def _run(self, sess, enqueue_op, coord=None):
    ...
    if isinstance(enqueue_op, (list, tuple)):
      # Assume profiling target ops are given one by one
      prof_target = False
    else:
      # Exclude default enqueue ops in slim/eval_image_classifier.py
      if 'parallel_read' in enqueue_op.name:
        prof_target = False
      elif 'batch' in enqueue_op.name:
        prof_target = False
      else:
        prof_target = True
    ...
    if prof_target:
      try:
        # Check and retrieve parameters related to profiling
        prof_iter_init = FLAGS.prof_iter_init
        prof_iter_period = FLAGS.prof_iter_period
        prof_iter_final = FLAGS.prof_iter_final
        logging.info('Writing trace files for sess.run(%s)' % enqueue_op.name)
        if prof_iter_period <= 0:
          logging.info('But prof_iter_period is negative... no traces for you :(')
          prof_target = False

      except AttributeError:
        logging.info('Was going to write trace files for sess.run(%s), but could not find prof_iter flags' % enqueue_op.name)
        prof_target = False

      
    try:
      # Perform profiling only if my enqueue_op is a profiling target
      if prof_target:
        enqueue_callable = sess.make_callable(enqueue_op, accept_options=True)
        i = -1
        if not os.path.exists('timeline-enqueue'):
          os.makedirs('timeline-enqueue')
      else:
      ...
      while True:
        ...
        if coord and coord.should_stop():
          break
        try:
          # Perform profiling only on certain iterations
          if prof_target and i >= prof_iter_init and i <= prof_iter_final and (i - prof_iter_init) % prof_iter_period == 0:
            run_metadata = config_pb2.RunMetadata()
            enqueue_callable(options=config_pb2.RunOptions(trace_level=config_pb2.RunOptions.FULL_TRACE),
                             run_metadata=run_metadata)
            trace = timeline.Timeline(step_stats=run_metadata.step_stats)
            with open('timeline-enqueue/timeline-%d.ctf.json' % i, 'w') as trace_file:
